...
ON = dict(TL.DEFAULTS, enabled=True)             # the rule as measured, switched on
CFG_ON = {"pdf_text_layer": {"enabled": True}}
...
def test_a_one_for_one_disagreement_marks_that_word_only():
    ...
    chk = TL.check(words, FILLER + ["02", "was", "measured"], ON)
...
def test_a_hyphen_joined_token_marks_every_word_in_it():
    ...
    chk = TL.check(words, FILLER + ["suspendod"], ON)
...
def test_a_many_to_one_disagreement_is_counted_never_marked():
    ...
    chk = TL.check(words, FILLER + ["into"], ON)
...
def test_agreement_marks_nothing_and_nothing_here_can_clear_a_flag():
    ...
    chk = TL.check(words, list(FILLER), ON)
...
def test_abstains_outside_the_measured_population():
    ...
    assert "outside the measured" in TL.check(words, FILLER + ["02"], ON, language="deu").abstained
    assert TL.check(words, FILLER + ["02"], ON, language="eng+deu").marked  # English is in it
    short = TL.check(words[-50:], FILLER[-49:] + ["02"], ON)
    assert short.marked == set() and "50 words" in short.abstained
    unrelated = TL.check(words, [f"other{i}" for i in range(201)], ON)
...
def test_apply_marks_the_words_in_place_across_sub_pages(tmp_path: Path):
    ...
    meta, notes = TL.apply(tmp_path, [left, right], CFG_ON, "eng")
...
def test_it_is_off_unless_switched_on(tmp_path: Path):
    """Switched off 2026-09-24 (re-judged on unflattened pages, it failed its
    per-document clause): no config section means OFF, not the measured rule."""
    TL.write_layer(tmp_path, FILLER + ["oxygen"], pdf_name="x.pdf", pdf_page=1)
    pg = _page(FILLER + ["cxygen"])
    meta, _ = TL.apply(tmp_path, [pg], {}, "eng")
    assert meta["words_marked"] == 0 and "disabled" in meta["abstained"]
    assert not any(w.layer_disagree for w in TL.page_words([pg]))
